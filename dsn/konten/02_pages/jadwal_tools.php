<?php
  ...
  if  (
        (!empty($_GET['ts'])) AND
        (!empty($_GET['id_kpta'])) AND
        (!empty($_GET['act']))
      ) {
          $ts = $_GET['ts'];
          $id_kpta = $_GET['id_kpta'];
          $act = $_GET['act'];
        }
  else {
    ?>
    ...
    elseif ($act == 'v')
      {
        $query_acc_jadwal = "UPDATE jadwal_kp SET
          status = 'Disetujui'
          WHERE id_jadwal_kp = '$id_kpta'
        ";
        if ($sql_acc_jadwal = $mysqli->query($query_acc_jadwal)) {
          ?>
          <script>
            alert('Data jadwal berhasil disetujui!');
            window.location.href="?p=jadwal_tools&ts=kp&id_kpta=<?php echo $id_kpta; ?>&act=detail";
          </script>
          <?php
        }
        else {
    ...
    elseif ($act == 'v')
      {
        $query_acc_jadwal = "UPDATE jadwal_ta SET
          status = 'Disetujui'
          WHERE id_jadwal_ta = '$id_kpta'
        ";
        if ($sql_acc_jadwal = $mysqli->query($query_acc_jadwal)) {
          ?>
          <script>
            alert('Data surat berhasil disetujui!');
            window.location.href="?p=jadwal_tools&ts=ta&id_kpta=<?php echo $id_kpta; ?>&act=detail";
          </script>
          <?php
        }
        else {
