...
<div class="row">
  <div class="col-lg-12">
    <form action="?a=sidang_aksi&ts=ta" method="post">
    ...
    <table class="table table-bordered table-hovered" id="dataTable" cellspacing=0 cellpadding=0>
      ...
      <?php
      $no=0;
      $query_pta_num = "SELECT * FROM jadwal_ta INNER JOIN mahasiswa INNER JOIN jurusan INNER JOIN dosen ON `mahasiswa`.`nim` = `jadwal_ta`.`nim` AND `mahasiswa`.`kode_jurusan` = `jurusan`.`kode_jurusan` AND `jadwal_ta`.`id_dosen` = `dosen`.`id_dosen` WHERE `jadwal_ta`.`status_sidang_ta` = 'Menunggu' LIMIT $w3";
      ...
      foreach ($sql_pta_num as $key) {
        ...
        ?>
          <tr>
            <td><input type="checkbox" id="id_jadwal_ta[]" name="id_jadwal_ta[]" value="<?php echo $id_jadwal_ta; ?>"></input></td>
            <td><?php echo $no; ?></td>
            <td><?php echo $nim; ?></td>
