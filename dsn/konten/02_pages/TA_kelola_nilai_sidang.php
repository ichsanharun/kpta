...
<div class="jumbotron" style="padding:10px">
  ...
  <div class="col-lg-8 collapse" id="collapseHead" aria-labelledby="headingOne" data-parent="#accordion">
    <?php
    $query_penilaian = "SELECT * FROM `sidang_ta_detail` WHERE `sidang_ta`.`id_sidang_ta` = '$_GET[id]'";
    $sql_penilaian = $mysqli->query($query_penilaian);
    $query_profil_mhs_sidang = "SELECT * FROM `sidang_ta` INNER JOIN jadwal_ta INNER JOIN mahasiswa INNER JOIN jurusan INNER JOIN dosen INNER JOIN ruang ON `sidang_ta`.`id_jadwal_ta` = `jadwal_ta`.`id_jadwal_ta`AND `jadwal_ta`.`nim` = `mahasiswa`.`nim` AND `mahasiswa`.`kode_jurusan` = `jurusan`.`kode_jurusan` AND `jadwal_ta`.`id_dosen` = `dosen`.`id_dosen` AND `sidang_ta`.`id_ruang` = `ruang`.`id_ruang` WHERE `sidang_ta`.`id_sidang_ta` = '$_GET[id]'";
