...
<div class="row">
  <div class="col-lg-12">
    <form action="?a=KP_absen_simpan" method="post">
      <input type="hidden" name="id_absen_kp" value="<?php echo $id_absen_kp; ?>">
    ...
    <table class="table table-bordered table-hovered" id="dataTable">
      ...
      <?php
      if (mysqli_num_rows($sql_kp_absen_mhs) > 0) {
        foreach ($sql_kp_absen_mhs as $key) {
          extract($key);
        }
      }
      else {
        foreach ($sql_kp_absen as $key => $value) {
          // code...
        }
      }
