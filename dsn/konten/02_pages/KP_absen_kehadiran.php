  ...
  <div data-toggle="collapse" data-target="#collapseHead">
    ...
    <div class="col-lg-8 collapse show" id="collapseHead" aria-labelledby="headingOne" data-parent="#accordion">
      <table class="table table-sm">
        <?php
        foreach ($sql_absen_kp_head as $key) {
          extract($key);
...
<div class="row">
  <div class="col-lg-12">
    <form action="?a=KP_absen_simpan" method="post">
    ...
    <table class="table table-bordered table-hovered" cellspacing="0" cellpadding="0" id="dataTable">
      ...
      <?php
      $no = 0;
      foreach ($sql_absen_kp_kehadiran as $key) {
        extract($key);
        $no++;
        ?>
          <tr>
    ...
    <button type="submit" name="selesai" value="<?php echo $id_absen_kp; ?>" class="btn btn-success"><i class="fa fa-fw fa-check"></i> Selesai Bimbingan</button>
    </form>
  </div>
</div>
<?php
foreach ($sql_absen_kp_list as $key) {
  extract($key);
  ?>
